...
const envSchema = z.object({
  ...
  API_PORT: z.preprocess((val) => val || '3001', z.string().transform((v) => parseInt(v, 10))),
  NODE_ENV: z.enum(['development', 'production', 'test']).default('development'),
  SLACK_WEBHOOK_URL: z.string().optional().or(z.literal('')),
  SMTP_HOST: z.string().optional(),
  SMTP_PORT: z.preprocess((val) => val || undefined, z.string().transform((v) => (v ? parseInt(v, 10) : undefined)).optional()),
