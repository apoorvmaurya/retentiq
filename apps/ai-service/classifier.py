...
class ChurnClassifier:
            ...
            try:
                ...
                if len(customers) >= 20:
                    ...
                    if len(np.unique(y)) >= 2:
                        ...
                        print(
                            f"[sklearn] Real training data contains only 1 class: {np.unique(y)}. Falling back to synthetic baseline dataset to ensure model can train."
                        )
                        X = []
                        y = []
    ...
    def get_shap_values(self, features_dict: dict) -> dict:
        ...
        try:
            ...
            return {name: float(val) for name, val in zip(self.feature_names, customer_shap, strict=False)}
        except Exception as e:
    ...
    def get_shap_values_batch(self, features_list: list[dict]) -> list[dict]:
        ...
        if not self.explainer:
            try:
                ...
                return [dict.fromkeys(self.feature_names, 0.0) for _ in features_list]
        ...
        try:
            ...
            for i in range(len(features_list)):
                ...
                sample_dict = {name: float(val) for name, val in zip(self.feature_names, sample_shap, strict=False)}
